...
namespace ITC.Core.Service
{
    public class AzureBlobStorageService : IAzureBlobStorageService
    {
        ...
        public async Task<AzureBlobStorageModel> DownloadAsync(string blobFilename)
        {
            ...
            try
            {
                ...
                if (await file.ExistsAsync())
                {
                    ...
                    return new AzureBlobStorageModel { Content = blobContent, Name = name, ContentType = contentType };
                }
            }
            catch (RequestFailedException ex)
                when (ex.ErrorCode == BlobErrorCode.BlobNotFound)
        ...
        public async Task<List<AzureBlobStorageModel>> ListAsync()
        {
            // Get a reference to a container named in appsettings.json
            BlobContainerClient container = new BlobContainerClient(_storageConnectionString, _storageContainerName);

            // Create a new list object for 
            List<AzureBlobStorageModel> files = new List<AzureBlobStorageModel>();
            ...
            await foreach (BlobItem file in container.GetBlobsAsync())
            {
                ...
                files.Add(new AzureBlobStorageModel
                {
                    Uri = fullUri,
                    Name = name,
        ...
        public async Task<BlobResponseDto> UploadListAsync(List<IFormFile> file)
        {
            BlobResponseDto response = new();
            BlobContainerClient container = new BlobContainerClient(_storageConnectionString, _storageContainerName);
            await container.CreateIfNotExistsAsync();
